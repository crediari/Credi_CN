...
const hexToRgb = (hex: string): [number, number, number] => {
  ...
  const normalized =
    value.length === 3 ? value.replaceAll(/./g, (channel) => channel + channel) : value;
...
void main() {
  ...
  vec3 backdrop = mix(vec3(0.027, 0.067, 0.051), vec3(1.0), step(0.5, uLightMode));
  ...
  if (uLightMode > 0.5) {
    ...
    float fibers = pow(smoothstep(0.1, 0.95, fiberField) * edgeFade, 1.2);
    outputColor = mix(backdrop, uLineColor, fibers * 0.62);
  } else {
...
type GhostFibersUniforms = {
  uResolution: ColorUniform;
  uTime: NumericUniform;
  uSpeed: NumericUniform;
  uScale: NumericUniform;
  uRotation: NumericUniform;
  uRotationSpeed: NumericUniform;
  uLayers: NumericUniform;
  uWaveAmplitude: NumericUniform;
  uWaveFrequency: NumericUniform;
  uWaveSpeed: NumericUniform;
  uLayerSpeed: NumericUniform;
  uTwist: NumericUniform;
  uTwistFrequency: NumericUniform;
  uTwistSpeed: NumericUniform;
  uLineFrequency: NumericUniform;
  uLineSpacing: NumericUniform;
  uLineSharpness: NumericUniform;
  uGlowFalloff: NumericUniform;
  uGlowIntensity: NumericUniform;
  uBrightness: NumericUniform;
  uBlueBoost: NumericUniform;
  uVignette: NumericUniform;
  uGrain: NumericUniform;
  uLightMode: NumericUniform;
...
const GhostFibers: FC<GhostFibersProps> = ({
  ...
  useEffect(() => {
    ...
    const uniforms: GhostFibersUniforms = {
      ...
      uGrain: { value: 0.05 },
      uLightMode: { value: 0 },
      uLineColor: { value: new Float32Array(hexToRgb("#06221c")) },
      uGlowColor: { value: new Float32Array(hexToRgb("#0f6b62")) },
    };
...
export { GhostFibers, type GhostFibersProps };
